...
function layout_head($titulo, $logado = false) {
    ...
    echo "<title>{$t} · Luis Imóveis Admin</title><style>" . layout_css() . '</style></head><body>';
    echo '<header class="top"><a class="brand" href="index.php">Luis Imóveis<span>admin</span></a>';
    if ($logado) {
...
.brand span{color:#1D9137;margin-left:7px;font-weight:600}
...
nav .who{color:#1D9137;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.08em}
nav .out{color:#b4543f}
.badge{font-style:normal;background:#1D9137;color:#fff;font-size:11px;font-weight:800;padding:1px 7px;border-radius:999px;margin-left:3px}
...
.tile:hover{border-color:#1D9137;transform:translateY(-2px);box-shadow:0 12px 30px -18px rgba(31,29,26,.4)}
...
input:focus,select:focus,textarea:focus{border-color:#1D9137;box-shadow:0 0 0 3px rgba(29,145,55,.15)}
...
.btn{display:inline-flex;align-items:center;gap:8px;padding:11px 18px;background:#1D9137;color:#fff;border:none;border-radius:10px;font-weight:700;font-size:14px;font-family:inherit;cursor:pointer;text-decoration:none;transition:.15s}
.btn:hover{background:#136127}
.btn.ghost{background:transparent;border:1px solid #DDE6DE;color:#5b544a}
.btn.ghost:hover{background:#fff;border-color:#1D9137}
...
code{background:#F4F7F5;padding:3px 8px;border-radius:6px;font-size:13px;color:#136127;font-weight:700}
...
.tag.destaque{background:#e3f1e6;color:#136127}
...
.iscapa{outline:2px solid #1D9137;outline-offset:-2px}
